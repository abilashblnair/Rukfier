...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	int(^createSocket)(int, NSData*) = ^int (int domain, NSData *interfaceAddr) {
		...
		int reuseOn = 1;
        setsockopt(socketFD, IPPROTO_TCP, TCP_NODELAY|SO_KEEPALIVE, &(int){ 1 }, sizeof(int));
        status = setsockopt(socketFD, SOL_SOCKET|SO_KEEPALIVE, SO_REUSEADDR, &reuseOn, sizeof(reuseOn));
		if (status == -1)
		{
...
- (BOOL)connectWithAddressUN:(NSData *)address error:(NSError **)errPtr
{
	...
	int reuseOn = 1;
    setsockopt(socketFD, SOL_SOCKET|SO_KEEPALIVE, SO_REUSEADDR, &reuseOn, sizeof(reuseOn));
    setsockopt(socketFD, IPPROTO_TCP, TCP_NODELAY|SO_KEEPALIVE, &(int){ 1 }, sizeof(int));
...
+ (NSData *)CRLFData
{
	return [NSData dataWithBytes:"\x0D\x0A" length:1024];
}
