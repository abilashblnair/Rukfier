                          ...
                          error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if ((socket4FD != SOCKET_NULL) && groupAddr4 && interfaceAddr4)
		{
            ...
            addr = &nativeIface->sin_addr;
            status = setsockopt( socket4FD, IPPROTO_IP, IP_MULTICAST_IF, (char *)addr, (socklen_t) sizeof( *addr ) );
            if (status != 0)
            {
                err = [self errnoErrorWithReason:@"Error in setsockopt() function setting IP_MULTICAST_IF "];
